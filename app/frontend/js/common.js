$(function(){
	setInterval(function(){
		...
		// 		today = new Date(),
		var	left = new Date(2017,05,31,23,59,59) - new Date();
	...
	// 	});
	// }
	var inpName = $('form input[name="name"]').on('blur', function(){ validate(inpName, /([a-zа-яіїєё -])/ig) });

	function validate(elem, pattern) {
		if ( elem.val().match(pattern) ) console.log('it`s ok');
		else {
			elem.css("border-color", "red");
			alert("Запрещенные символы!");
		}
	}
